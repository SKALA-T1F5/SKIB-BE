...
@RestController
@Tag(name = "Document API", description = "문서 정보 관련 API")
@RequiredArgsConstructor
public class DocumentController {

  private final DocumentService documentService;

  @SwaggerApiSuccess(summary = "문서 정보 저장", description = "문서 정보를 저장합니다.")
  @SwaggerApiNotFoundError
  @SwaggerInternetServerError
  @PostMapping("/api/document")
  public ResponseEntity<?> createDocument(
      @RequestParam("projectId") Integer projectId, @RequestBody RequestCreateDocumentDto dto) {

    documentService.saveDocument(projectId, dto);
    return ResponseEntity.ok(ResultDto.res(HttpStatus.OK, "SUCCESS", "문서 정보 저장 완료"));
  }
